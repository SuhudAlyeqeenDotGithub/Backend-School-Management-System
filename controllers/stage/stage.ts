...
import { Stage } from "../../models/curriculum/stage";
...
export const getStages = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (absoluteAdmin || hasAccess) {
    const stages = await Stage.find({ organisationId: organisation!._id.toString() });
    ...
    if (!stages) {
      ...
      throwError("Error fetching stage", 500);
    }
  ...
  throwError("Unauthorised Action: You do not have access to view stage - Please contact your admin", 403);
});
...
export const createStage = asyncHandler(async (req: Request, res: Response) => {
  ...
  const stageExists = await Stage.findOne({ organisationId: orgParsedId, customId });
  if (stageExists) {
    registerBillings(req, [
      { field: "databaseOperation", value: 4 },
      {
        field: "databaseDataTransfer",
        value: getObjectSize([stageExists, organisation, role, account])
      }
    ]);
    throwError(
  ...
  if (!newStage) {
    registerBillings(req, [
      ...
      { field: "databaseDataTransfer", value: getObjectSize([stageExists, newStage, organisation, role, account]) }
    ]);
    throwError("Error creating stage - Please try again", 500);
  }
...
export const updateStage = asyncHandler(async (req: Request, res: Response) => {
  ...
  const originalStage = await Stage.findOne({ organisationId: orgParsedId, customId });

  if (!originalStage) {
    registerBillings(req, [
      { field: "databaseOperation", value: 4 },
      { field: "databaseDataTransfer", value: getObjectSize([originalStage, organisation, role, account]) }
    ]);
